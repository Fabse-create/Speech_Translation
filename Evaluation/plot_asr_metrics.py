...
def plot_metrics(metrics_path: Path, output_dir: Path) -> None:
    ...
    ax.set_title("ASR Training Loss")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.legend()
    fig.savefig(output_dir / "asr_loss.png", bbox_inches="tight")

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(epochs, val_wer, label="val_wer")
    ax.set_title("ASR Validation WER")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("WER")
    ax.set_ylim(0.0, 1.0)
    ax.legend()
    fig.savefig(output_dir / "asr_wer.png", bbox_inches="tight")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot ASR training metrics.")
    parser.add_argument(
        "--metrics-path",
        default="Evaluation/asr_training_results/metrics.json",
        help="Path to ASR training metrics.json.",
    )
    parser.add_argument(
        "--output-dir",
        default="Evaluation/asr_training_results",
